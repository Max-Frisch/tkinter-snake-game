...
class Game(ctk.CTk):
	...
	def get_input(self, event):
		match event.keycode:
			case 83: self.direction = DIRECTIONS['left'] if self.direction != DIRECTIONS['right'] else self.direction
			case 80: self.direction = DIRECTIONS['up'] if self.direction != DIRECTIONS['down'] else self.direction
			case 85: self.direction = DIRECTIONS['right'] if self.direction != DIRECTIONS['left'] else self.direction
			case 88: self.direction = DIRECTIONS['down'] if self.direction != DIRECTIONS['up'] else self.direction
